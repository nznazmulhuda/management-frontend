...
import { multipleSellSchema } from "@/components/AddNewSell/sellDataVelidator"
import { onMultipleDataSubmit } from "@/services/sells/addSellData"

const formFieldNames = ["customer_name", "customer_number", "customer_address", "quantity", "total_price", "total_costing"] as const;
...
export const formField: { name: FormFieldName, label: string, placeholder: string, formDescription: string, isTextArea: boolean, }[] = [
  ...
  },
  {
    ...
    label: 'Quantity',
    placeholder: 'Ex: 2',
    formDescription: 'Enter the total quantity.',
    isTextArea: false,
  },
  {
    name: 'total_price',
    label: 'Total price',
    placeholder: 'Ex: 700',
    formDescription: 'Enter the total price of t-shirts.',
    isTextArea: false,
  },
  {
    name: 'total_costing',
    label: 'Total costing',
    placeholder: 'Ex: 320',
    formDescription: 'Enter the total costing of t-shirts.',
    isTextArea: false,
  },
]

function AddMultipleSellData() {
  // Define your form.
  const multipleDataForm = useForm<z.infer<typeof multipleSellSchema>>({
    resolver: zodResolver(multipleSellSchema),
    defaultValues: {
      size: "M",
      color: "black",
      quantity: '2',
  ...
  return (
    <Form {...multipleDataForm}>
      <form onSubmit={multipleDataForm.handleSubmit(onMultipleDataSubmit)} className="space-y-4">
        {
          formField.map((fieldData) => fieldData.isTextArea ?
            <FormField
              control={multipleDataForm.control}
              name={fieldData.name}
              render={({ field }) => (
                <FormItem>
            ...
            render={({ field }) => (
              <FormItem className="w-1/2">
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  <FormControl>
                    <SelectTrigger>
                      <SelectValue placeholder="Ex: White" className="placeholder:text-white" />
                    </SelectTrigger>
                  </FormControl>
                  <SelectContent>
                    {
...
export default AddMultipleSellData